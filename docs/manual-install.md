...
```bash
helm install audit oci://registry-1.docker.io/multicloud/advisor-chart --version 0.5.3 \
  -n advisor \
...
```bash
helm show values oci://registry-1.docker.io/multicloud/advisor-chart --version 0.5.3
```

What now *enforces* it is `values.schema.json`, which ships inside the chart — read it with
`helm pull oci://registry-1.docker.io/multicloud/advisor-chart --version 0.5.3 --untar`.
...
|---|---|---|
| Cluster | nodes, pods, services | get, list, watch |
...
```bash
helm upgrade audit oci://registry-1.docker.io/multicloud/advisor-chart --version 0.5.3 \
...
printf %s "$AWS_SECRET_ACCESS_KEY" | kubectl -n advisor create secret generic advisor-quota-aws \
  --from-literal=AWS_ACCESS_KEY_ID=<KEY_ID> --from-file=AWS_SECRET_ACCESS_KEY=/dev/stdin
...
Same shape as the catalog key in Step 1, and for the same reason: the secret arrives on stdin and
never enters the command's `argv`, where `ps`, `/proc/<pid>/cmdline` and execve auditing would all
pick it up — see [permissions.md § Keep secret values out of the command
line](permissions.md#keep-secret-values-out-of-the-command-line). The key **id** stays on
`--from-literal`: it identifies the caller in your own CloudTrail logs rather than authenticating
as one. Per cloud, that leaves exactly one value needing stdin — `AZURE_CLIENT_SECRET`, with
`AZURE_TENANT_ID`, `AZURE_CLIENT_ID` and `AZURE_SUBSCRIPTION_ID` staying literals as the
identifiers they are; and none at all for Google Cloud, where
`--from-file=GCE_SA_KEY_JSON=<path/to/key.json>` passes a path.
